...
public class CheckoutActivity extends AppCompatActivity {
    ...
    private ArrayList<TanggalKirim> listTanggalKirim;
                ...
                public void onResponse(Call<JsonObject> call, Response<JsonObject> response) {
                    if(response.isSuccessful()){
                        ...
                        if(status.equals("success")){
                            ...
                            checkoutCompanyAdapter = new CheckoutCompanyAdapter(CheckoutActivity.this, listCheckoutCompany, total, listongkir, listAlamat, listPayment, listAlamatBillto, listLibur, listNote, listTanggalKirim);
                ...
                public void onResponse(Call<JsonObject> call, Response<JsonObject> response) {
                    if (response.isSuccessful()){
                        ...
                        if (status.equals("success")){
                            ...
                            Log.d(TAG, "Kesalahan di get payment");
                        }
                    }else{
                        Log.d(TAG, "Kesalahan di get payment");
                    }
                ...
                public void onResponse(Call<JsonObject> call, Response<JsonObject> response) {
                    if(response.isSuccessful()){
                        ...
                        if(status.equals("success")){
                            ...
                            Log.d(TAG, "Kesalahan di get shipto");
                        }
                    }else{
                        Log.d(TAG, "kesalahan di get shipto");
                    }
                ...
                public void onResponse(Call<JsonObject> call, Response<JsonObject> response) {
                    if(response.isSuccessful()){
                        ...
                        if(status.equals("success")){
                            ...
                            Log.d(TAG, "Kesalahan di get billto");
                        }
                    }else{
                        Log.d(TAG, "Kesalahan di get billto");
                    }
...
//        String get_kalender = "SELECT * FROM gcm_kalender_libur where tanggal >= now() - interval '1day' order by tanggal asc";
        String get_kalender = "SELECT * FROM gcm_kalender_libur";
        try{
                ...
                public void onResponse(Call<JsonObject> call, Response<JsonObject> response) {
                    if (response.isSuccessful()){
                        ...
                        if (status.equals("success")){
                            ...
                            Log.d(TAG, "Kesalahan di get kalender libur");
                            loadingDialog.hideDialog();
                            Toast.makeText(getApplicationContext(), "Tidak dapat mengabil data kalender libur", Toast.LENGTH_LONG).show();
                        }
                    }else{
                        Log.d(TAG, "Kesalahan di get kalender libur");
                        loadingDialog.hideDialog();
                        Toast.makeText(getApplicationContext(), "Gagal mengabil data kalender libur", Toast.LENGTH_LONG).show();
                    }
                }
...
//                            mSocket.emit("new_transaction", test);
    ...
    private void getTanggalPermintaanKirim(){
        String query = "select a.master_cart_id, a.line_id,to_char(a.tgl_permintaan_kirim, 'yyyy-MM-dd') as tgl_permintaan_kirim," +
                "initcap(to_char(a.tgl_permintaan_kirim, 'dd-mon-yyyy')) as tgl_permintaan_kirim_edit, " +
                "a.qty as qty_kirim, a.tipe_pengiriman, c.company_id, d.nama_perusahaan, b.barang_id from gcm_master_cart b " +
                "left join gcm_tanggal_kirim a on a.master_cart_id = b.id " +
                "inner join gcm_list_barang c on c.id = b.barang_id inner join gcm_master_company d on d.id = c.company_id " +
                "where b.company_id = " + SharedPrefManager.getInstance(getApplicationContext()).getUser().getCompanyId() + " and b.status = 'A' order by a.master_cart_id, a.tgl_permintaan_kirim asc";
        Log.d(TAG, "getTanggalPermintaanKirim: "+query);
        try {
            Call<JsonObject> getTanggalpermintaanKirim = RetrofitClient
                    .getInstance()
                    .getApi()
                    .request(new JSONRequest(QueryEncryption.Encrypt(query)));
            getTanggalpermintaanKirim.enqueue(new Callback<JsonObject>() {
                @Override
                public void onResponse(Call<JsonObject> call, Response<JsonObject> response) {
                    if (response.isSuccessful()){
                        String status = response.body().getAsJsonObject().get("status").getAsString();
                        if (status.equals("success")){
                            ...
                            for (int i=0; i<jsonArray.size(); i++){
                                JsonObject jsonObject = jsonArray.get(i).getAsJsonObject();
                                if (jsonObject.get("master_cart_id").isJsonNull() ||
                                        jsonObject.get("line_id").isJsonNull() ||
                                        jsonObject.get("tgl_permintaan_kirim_edit").isJsonNull() ||
                                        jsonObject.get("qty_kirim").isJsonNull() ||
                                        jsonObject.get("tipe_pengiriman").isJsonNull() ||
                                        jsonObject.get("tgl_permintaan_kirim_edit").isJsonNull()){
                                    int master_cart_id = 0;
                                    int line_id = 0;
                                    String tgl_permintaan_kirim_edit = "dd/mm/yyyy";
                                    int qty_kirim = 0;
                                    String tipe_pengiriman = "";
                                    String nama_perusahaan = jsonObject.get("nama_perusahaan").getAsString();
                                    int barang_id = jsonObject.get("barang_id").getAsInt();
                                    int company_id = jsonObject.get("company_id").getAsInt();
                                    listTanggalKirim.add(new TanggalKirim(line_id, master_cart_id, tgl_permintaan_kirim_edit, qty_kirim, tipe_pengiriman, company_id, barang_id, nama_perusahaan));
                                }else {
                                    int master_cart_id = jsonObject.get("master_cart_id").getAsInt();
                                    int line_id = jsonObject.get("line_id").getAsInt();
                                    String tgl_permintaan_kirim_edit = jsonObject.get("tgl_permintaan_kirim_edit").getAsString();
                                    int qty_kirim = jsonObject.get("qty_kirim").getAsInt();
                                    String tipe_pengiriman = jsonObject.get("tipe_pengiriman").getAsString();
                                    String nama_perusahaan = jsonObject.get("nama_perusahaan").getAsString();
                                    int barang_id = jsonObject.get("barang_id").getAsInt();
                                    int company_id = jsonObject.get("company_id").getAsInt();
                                    listTanggalKirim.add(new TanggalKirim(line_id, master_cart_id, tgl_permintaan_kirim_edit, qty_kirim, tipe_pengiriman, company_id, barang_id, nama_perusahaan));
